...
class Clientadmin extends CI_Controller {
	...
	public function fundsweep_config($id){
		...
		if($this->input->post('update_fs_configuration') != '')
		{
			...
			if ($save['vat_computation_hold'] <= 20 && $save['sweep_execution_day'] <= 28 && $save['sweep_execution_day'] >= 25) {
			...
			$this->session->set_flashdata('error',"Client computation date should not be greater than 20th and sweep date should not be greater than 27th or less than 24th.");			# code...
			redirect(getUrl('clientadmin/fundsweep_config'));
		}
